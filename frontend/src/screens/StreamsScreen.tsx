...
export function StreamsScreen({ snapshot, frontendStatuses, reportStatus, serverStatus }: ScreenProps) {
        ...
        <div ref={screenRef} className="relative flex h-screen w-screen flex-col overflow-hidden bg-black text-white">
            <div ref={gridRef} className="grid min-h-0 w-full flex-1" style={gridStyle}>
                {snapshot.streams.map((stream) => (
                    <StreamTile key={stream.id} stream={stream} frontendStatus={frontendStatuses[stream.id]} reportStatus={reportStatus} />
                ))}
            </div>
